...
class LibriSpeechAugmented(LIBRISPEECH):
    ...
    def __getitem__(self, index: int):
        ...
        #print("after padding 2 shape:", spec.shape)

        #print("after augmentation:", spec.shape)

        return spec.T, transcript, length
...
class MyPipeline(torch.nn.Module):
    ...
    def forward(self, waveform: torch.Tensor) -> torch.Tensor:
        ...
        #print("after stretch:", spec.shape)

        spec = spec.abs().pow(2)
        #print("after abs().pow(2):", spec.shape)
        # Apply SpecAugment
        spec = self.spec_aug(spec)
        #print("after spec_aug:", spec.shape)

        # Convert to mel-scale
        mel = self.mel_scale(spec)
        #print("after mel_scale:", mel.shape)
